...
export function ServiceForm({ children, emailserviceid, emailtemplateid, publickey, captchakey, formid, _next}) {
...
export function SuggestionForm() {
    ...
    const handleChange = event => {
        navigate(`/${locale}/services`, {state: { selectedServiceId: event.target.value }});
    };

    return (
        <div className="suggestion-form">
            <h1>{translate(locale, "form_suggestion_title")}</h1>
            <div>

                <span className="form-field" style={{ marginBottom: "24px" }}>
                    <select defaultValue="default" form="suggestion-form" id="service" name="Serviciu" onChange={handleChange} required>
                        <option disabled hidden value="default">{translate(locale, "form_selectaservice")}</option>
                        <option value="car-damage">{translate(locale, "services_car_damage")}</option>
                        <option value="air-conditioning">{translate(locale, "services_air_conditioning")}</option>
                        <option value="mechanics">{translate(locale, "services_mechanics")}</option>
                        <option value="tinware">{translate(locale, "services_tinware")}</option>
                        <option value="wheel-geometry">{translate(locale, "services_wheel_geometry")}</option>
                        <option value="paint-shop">{translate(locale, "services_paint_shop")}</option>
                        <option value="headlights">{translate(locale, "services_headlights")}</option>
                        <option value="computerized-test">{translate(locale, "services_computerized_test")}</option>
                    </select>
                </span>
...
/*
                        <input onChange={handleInput} form={formid} type="number" id={id_year} name="year" min="1900" max="2100" required placeholder={translate(locale, "form_placeholder_year")} />
 */
